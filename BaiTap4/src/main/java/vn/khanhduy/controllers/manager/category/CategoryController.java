package vn.khanhduy.controllers.manager.category;
...
@WebServlet(urlPatterns = { "/manager/home", "/manager/add", "/manager/edit", "/manager/delete" })
...
public class CategoryController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * 
		 * req.getServletPath() -> /manager/add
		 */
		String path = req.getServletPath();// hoặc req.getRequestURI()
		if (path.startsWith("/manager/add")) {
			doGetAdd(req, resp);
		} else if (path.startsWith("/manager/edit")) {
			doGetEdit(req, resp);
		} else if (path.startsWith("/manager/delete")) {
	...
	protected void doGetList(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		Users user = (Users) SessionUtil.getInstance().getValue((HttpServletRequest) req, "USERMODEL");
		List<Category> cateList = cateService.getAllForManager(user.getId());
		req.setAttribute("cateList", cateList);
		RequestDispatcher dispatcher = req.getRequestDispatcher("/views/manager/list-category.jsp");
		dispatcher.forward(req, resp);
	}

	protected void doGetAdd(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		RequestDispatcher dispatcher = req.getRequestDispatcher("/views/manager/add-category.jsp");
		dispatcher.forward(req, resp);
	}
